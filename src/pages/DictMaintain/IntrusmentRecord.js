...
class IntrusmentRecord extends PureComponent {
  state = {
    visible: false,
  ...
  componentDidMount() {
    ...
    const diviceName = sessionStorage.getItem('diviceName');
    const user = JSON.parse(localStorage.getItem("userinfo"));
    dispatch({
      type: 'intrusment/getInstrumentRecord',
      payload: {
        diviceName,
  ...
  deleteItem = text => {
    ...
    dispatch({
      type: 'intrusment/deleteInstrumentRecord',
      payload: {
        keyno:text.keyno,
      },
      callback: (response) => {
  ...
  handleOk = () => {
    ...
    validateFieldsAndScroll((error, values) => {
      if (!error) {
        ...
        formData.append('diviceName', diviceName);
        formData.append('filetype', "仪器文件");
        formData.append('recordname', values.recordname);
        dispatch({
          type: 'intrusment/uploadInstrumentRecord',
          payload: formData,
          callback: (response) => {
            if (response.code === 400) {
              notification.open({
...
export default IntrusmentRecord;