...
class CertificateMakeDetail extends PureComponent {
  state = {
    ...
    dispatch({
        ...
        callback: (response) => {
          if (response.code === 200) {
            ...
            const wpsUrl = `https://www.smlq.vip/wps/edit/certificate?_w_signature=${_w_signature}&_w_userid=${_w_userid}&_w_fname=${_w_fname}`;
            window.open(wpsUrl);
          } else {
            notification.open({
              message: '加载失败',
...
export default CertificateMakeDetail;
