...
@Form.create()
class InspmanDetail extends PureComponent {
  state = {
    selectedRowKeys: [],
  ...
  componentDidMount() {
    this.init();
    this.setState({overallstate:sessionStorage.getItem('overallstate_InspmanDetail')});
  }
      ...
      callback: (response) => {
        if (response){
          if(response.list===null || response.list.length===0){
            Modal.info({
              title: '检测人员信息未配置',
              content:'请管理员在“公司管理-用户管理”菜单配置，并在角色中加选检测人员角色！',
              okText:"知道了",
              onOk() {
              },
            });
